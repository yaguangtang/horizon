...
from openstack_dashboard.dashboards.project.metering import tables as \
    metering_tables
from openstack_dashboard.dashboards.project.metering import tabs as \
    metering_tabs


class IndexView(tabs.TabbedTableView):
    tab_group_class = metering_tabs.CeilometerOverviewTabs
    template_name = 'project/metering/index.html'


class SamplesView(TemplateView):
    template_name = "project/metering/samples.csv"
...
class ReportView(tables.MultiTableView):
    template_name = 'project/metering/report.html'
...
def query_data(request,
    ...
    query = {}
    query[request.user.tenant_name] = [{
                            "field": "project_id",
                            "op": "eq",
                            "value": request.user.tenant_id}]
